...
def saveObj(self, filedir = './Pickles/', filenamebase = 'MockGridTemp'):
    if not os.path.exists(filedir):
        os.makedirs(filedir)
    vns = self.__dict__.keys()
    vnfile = open(filedir + filenamebase + '_varnames'  + '.pickle', 'w')
    for vn in vns:
        vnfile.write('{0}\n'.format(vn))
        with open(filedir + filenamebase + '_' + vn + '.pickle', 'wb') as handle:
            pickle.dump(self.__dict__[vn], handle, protocol=pickle.HIGHEST_PROTOCOL)

def loadObj(self, filedir = './Pickles/', filenamebase = 'MockGridTemp'):
    ...
    for vn in vnfile.readlines():
        ...
        with open(filedir + filenamebase + '_' + vnstrip + '.pickle', 'rb') as handle:
                self.__dict__[vnstrip] = pickle.load(handle)

def loadElementsFromPickle(self, filedir = './Pickles/', filenamebase = 'MockGridTemp',
        ...
        if e in vnsLoaded:
            if overwriteElements:
                ...
                with open(filedir + filenamebase + '_' + e + '.pickle', 'rb') as handle:
                    self.__dict__[e] = pickle.load(handle)
            else:
...
def saveElementsAsPickle(self, filedir = './Pickles/', filenamebase = 'MockGridTemp', 
                        listOfElements = ['ells'], overwriteElements = False):
    if not os.path.exists(filedir):
        os.makedirs(filedir)
        ...
        if os.path.isfile(pfname):
            if overwriteElements:
                ...
                with open(filedir + filenamebase + '_' + e + '.pickle', 'wb') as handle:
                    pickle.dump(self.__dict__[e], handle, protocol=pickle.HIGHEST_PROTOCOL)
            else:
